...
number_of_houses=5
houses=[i for i in range(0,number_of_houses)]
number_of_days=1000
...
def simulate(cartel_of_neighbours,
             ...
             central_energy_capacity,
             energy_decay_multiplier,
             solar_panel_efficiency,
             house_max_storage,
             average_energy_usage_per_house,
             average_solar_panels_per_house,
             energy_price_local,
             energy_price_external_grid,
             number_central_solar_panels,
             annual_central_wind_energy_production):
    #tokens per producer
    tokens=[0 for i in range(0,number_of_houses+1)]

    #tokens paid to external grid
    paid_to_external_grid=0
    ...
    effective_central_solar_panels=number_central_solar_panels*solar_panel_efficiency
    ...
    for t in range(0, number_of_days*time_slots_per_day):
        ...
        # randomize order of access to grid
        random.shuffle(houses)

        for house in houses:
            usage=get_usage(house, t,base_hourly_energy_usage)
            production=get_house_production(house,t,effective_solar_panels_per_house)
            current_household_storage, tokens, central_energy_contributions,energy_price_local,paid_to_external_grid=house_simulation_1_time_unit(house,production,usage,tokens,current_household_storage,central_energy_capacity,central_energy_contributions,energy_price_local,energy_price_external_grid,max_storage_per_house,cartel_of_neighbours,cartel_price_multiplier,paid_to_external_grid)
    ...
    print("cartel:", cartel_of_neighbours, "cartel energy price:",energy_price_local*cartel_price_multiplier ,"tokens: ",tokens)
...
def house_simulation_1_time_unit(house_number,
                                 production,
                                 usage,
                                 tokens,
                                 current_household_storage,
                                 central_energy_capacity,
                                 central_energy_contributions,
                                 energy_price_local,
                                 energy_price_external_grid,
                                 max_storage_per_house,
                                 cartel_of_neighbours,
                                 cartel_price_multiplier,
                                 paid_to_external_grid):
    net_usage=usage-production

    # first, always try to top up house battery as much as possible from central contributions
    # this only makes sense when there is a cartel
    # without cartel, houses prefer to receive money for energy rather than letting it decay
    if central_energy_contributions[house_number]>0 :
        energy_to_add_to_house_storage=min(max_storage_per_house[house_number]-current_household_storage[house_number],central_energy_contributions[house_number])
        current_household_storage[house_number]=current_household_storage[house_number]+energy_to_add_to_house_storage
        central_energy_contributions[house_number]=central_energy_contributions[house_number]-energy_to_add_to_house_storage
            ...
            if house_number in cartel_of_neighbours:
                for seller in cartel_of_neighbours:
                    if current_household_storage[seller]>0:
                        net_usage, tokens, current_household_storage=neighbour_buy(net_usage,house_number,seller,tokens,energy_price_local,current_household_storage,cartel_price_multiplier)
            

            if net_usage>0:
                tokens, central_energy_contributions, energy_price_local,paid_to_external_grid=market_buy(net_usage,house_number,tokens, central_energy_contributions,energy_price_local,energy_price_external_grid,paid_to_external_grid)
    ...
    return current_household_storage, tokens, central_energy_contributions,energy_price_local, paid_to_external_grid
...
def market_buy(energy_to_buy,
    ...
    if energy_to_buy>central_energy_contributions[house_number]:
        energy_to_buy=energy_to_buy-central_energy_contributions[house_number]
    ...
    if energy_to_buy>local_storage_total:
        ...
        amount_to_pay_externally=(energy_to_buy-local_storage_total)*energy_price_external_grid
        paid_to_external_grid+=amount_to_pay_externally
        cost=amount_to_pay_externally+local_storage_total*energy_price_local
        tokens[house_number]=tokens[house_number]-cost
        for i in range(0,number_of_houses+1):
            #pay houses who contributed to energy being bought
            tokens[i]=tokens[i]+(central_energy_contributions[i]*energy_price_local)
            #remove that energy from central supply
            central_energy_contributions[i]=0
        ...
        cost=energy_to_buy*energy_price_local
        tokens[house_number]=tokens[house_number]-cost
        #discount the contributions of each house to local storage
        for i in range(0,number_of_houses+1):
            #pay houses who contributed to energy being bought
            tokens[i]=tokens[i]+(central_energy_contributions[i]*pct_to_buy*energy_price_local)
            #remove that energy from central supply
            central_energy_contributions[i]=central_energy_contributions[i]*(1-pct_to_buy)
    
    return tokens, central_energy_contributions, energy_price_local, paid_to_external_grid


#allows neighbours to buy from each other directly
def neighbour_buy(energy_to_buy,house_buying, house_selling,tokens, energy_price_local,current_household_storage,cartel_price_multiplier):
    ...
    if energy_to_buy>current_household_storage[house_selling]:
        ...
        tokens[house_buying]=tokens[house_buying]-(current_household_storage[house_selling]*(energy_price_local*cartel_price_multiplier))
        tokens[house_selling]=tokens[house_selling]+(current_household_storage[house_selling]*(energy_price_local*cartel_price_multiplier))
        current_household_storage[house_selling]=0
        
    #seller has sufficient energy
    else:
        current_household_storage[house_selling]=current_household_storage[house_selling]-energy_to_buy
        tokens[house_buying]=tokens[house_buying]-(energy_to_buy*(energy_price_local*cartel_price_multiplier))
        tokens[house_selling]=tokens[house_selling]+(energy_to_buy*(energy_price_local*cartel_price_multiplier))
        energy_to_buy=0
...
# if this is 0, central production energy dominates
...
#central storage and production variables
